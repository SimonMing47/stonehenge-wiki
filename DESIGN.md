...
- 机器可读 API contract：`GET /api/contract` 和 Rust CLI `--api-contract` 暴露 route、scope、query/body、CLI 映射与 no-RAG 边界。
...
| API contract | `PYTHONPATH=work python3 -m stonehenge_wiki.contract_checks`，校验 route、scope、query/body、CLI 映射 | A/C | 每 PR |
...
- 给 REST API route contract 增加更深的字段类型、必填项和响应 schema 一致性检查。
