...
**版本**：v1.1
**更新日期**：2026-07-02
**目标读者**：产品负责人、后端/平台工程师、前端工程师、测试/交付工程师、OpenCode 运行入口维护者
...
- OpenCode Wrapper：面向本地 OpenCode runtime 的能力封装。
...
- LLM 配置：支持 OpenCode 独立 agent、环境变量凭证注入和分类路由。
...
- release bundle：交付归档。
- traceable release manifest：记录生成者、artifact 清单、每个文件的 `size`/`sha256` 和发布包自身 `sha256`。
- Web Console：Ask、Wiki、Workbench、Raw、Agents、Governance、Audit。
- Rust REST CLI：Linux/Windows 入口和 skill 内二进制构建脚本。
- 机器可读 API contract：`GET /api/contract` 和 Rust CLI `--api-contract` 暴露 route、scope、query/body 字段 required/type/alias/enum 元数据、CLI 映射与 no-RAG 边界。
...
| Wrapper | `work/skills/stonehenge-wiki` | OpenCode 使用说明和 CLI 包装 | 不实现另一套能力 |
...
- API contract 统一由 `GET /api/contract` 暴露，schema v2 包含 route、scope、query/body 字段 required/type/alias/enum 元数据、CLI 映射和 no-RAG 架构边界。
...
- release bundle manifest 可追踪生成者、artifact 清单和 `size`/`sha256`。
...
- release bundle manifest 可追溯，且每个 artifact 有 `size`/`sha256`。
...
- OpenCode provider/model 配置可保存、测试、回滚。
...
| API contract | `PYTHONPATH=work python3 -m stonehenge_wiki.contract_checks`，校验 route、scope、query/body 字段 required/type/alias/enum、CLI 映射 | A/C | 每 PR |
...
| `work/skills/stonehenge-wiki/SKILL.md` | OpenCode wrapper 使用方式 | C |
