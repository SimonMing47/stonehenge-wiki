...
Karpathy 的 LLM Wiki 模式强调三层：raw sources、LLM-generated wiki、schema。当前项目据此新增编译型 Markdown wiki：`docs/` 是 raw sources，`wiki/` 是生成知识层，`AGENTS.md` 是 schema/维护契约。
...
- SQLite 保存可重建索引、批注元数据、来源注册表、metadata-only 来源版本历史、审计事件和任务运行记录，不复制原始文档正文，降低敏感内容扩散风险。
