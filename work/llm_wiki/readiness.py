...
def directory_gate(wiki_root: Path) -> dict[str, Any]:
    ...
    return gate(
        "directory_layout",
        "Required llm-wiki layout",
        "fail" if missing else "pass",
        "missing=" + ", ".join(missing) if missing else "all required paths exist",
        "Create the missing llm-wiki directories and contract files.",
    )
...
def cli_skill_gate(wiki_root: Path) -> dict[str, Any]:
    ...
    return gate(
        "cli_skill_surface",
        "CLI and skill entrypoints",
        "fail" if missing else "pass",
        "missing=" + ", ".join(missing) if missing else "CLI and skill wrappers present",
        "Restore work/main.py and the llm-wiki skill wrapper.",
    )
...
def no_rag_gate(health: dict[str, Any]) -> dict[str, Any]:
    ...
    return gate(
        "no_rag_architecture",
        "LLM Wiki architecture without RAG",
        "pass" if ok else "fail",
...
def persistence_audit_gate(wiki_root: Path, health: dict[str, Any]) -> dict[str, Any]:
    ...
    return gate(
        ...
        "fail" if failed else "pass",
        evidence,
        "Enable persisted index state and audit tables under llm-wiki/.state.",
    )
