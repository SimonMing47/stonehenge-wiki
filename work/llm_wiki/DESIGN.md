# LLM Wiki 详细设计文档（企业知识工作台）

**版本**：v1.0  
**更新日期**：2026-06-29  
**适用范围**：`/Users/mac/Documents/LLM-Wiki-System` 项目内 `work/llm_wiki` 平台  
...
客户端
  ...
  ├─ CLI  (work/llm_wiki/cli.py)
  └─ Skill Wrapper (work/skills/llm-wiki)
        │
        ▼
LLM Wiki Platform (work/llm_wiki/platform.py)
        │
        ├─ 配置与环境（config.py）
...
- **入口层**：CLI / HTTP / Skill 统一把参数转换为平台动作。
- **服务层**：`LLMWikiPlatform` 承载业务流转、校验与编排。
...
- `LLM_WIKI_API_TOKEN`：管理员操作（导入、重建、编译、题组执行、生成输出、导出）
- `LLM_WIKI_READ_TOKEN`：只读操作（查看索引、审计、问答、治理结果）
- 无 token 时按配置支持公开读写（默认不推荐）
- API 均以 `X-LLM-WIKI-TOKEN` 头传入
...
- `--serve`：启动 HTTP API（默认 `127.0.0.1:8765`）
...
健康与索引：
- `GET /health`, `GET /index`, `GET /audit`
...
- 启动：`python3 -m llm_wiki.cli --serve --wiki-root /path/to/llm-wiki --port 8765`
- 守护运行：外部用 `nohup/systemd/launchd` 等方式维持进程
...
- 可回归验证：
  ...
  - `python3 -m llm_wiki.cli --self-test`
...
- [x] CLI/skill/API/console 统一平台层
- [x] 来源治理与隔离机制
- [x] 审计与治理报告
- [x] 就绪度门禁与评估报告
- [x] Web 页面“工作台”与“LLM Wiki”命名统一
